...
def booking_resp(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print (booking.objects.values_list("roll_number",flat=True))
            if form.cleaned_data['roll_number'] not in booking.objects.values_list("roll_number",flat=True):
                for bic in booking.objects.all():
                    if bic.room_number == form.cleaned_data['room_number']:

                        bic.occupant = form.cleaned_data['occupant']
                        bic.roll_number = form.cleaned_data['roll_number']
                        bic.occupied = True

                        bic.save()
                    else:
                        pass
            elif form.cleaned_data['roll_number'] == "":
                pass
            else:
                result = 1
    ...
    return render_to_response("book.html", {"results" : result})
